...
def graph_reducer(G, cuttoff=50):
    ...
    total = sum(float(node_frequency[k]) for k in node_frequency.keys())
    # updating edge weight based on ZTest
    edges = list(G.edges())
    for e in edges:
        topicFreq_i = float(G.nodes[e[0]]["weight"])
        topicProb_j = float(G.nodes[e[1]]["weight"]) / total
        expected = topicFreq_i * topicProb_j
        if(expected > 0):
            ZTest = (float(G.edges[e]["weight"]) - expected)/(expected**0.5)
        else:
            ZTest = 100
        if ZTest < cuttoff:
            G.remove_edge(*e)
